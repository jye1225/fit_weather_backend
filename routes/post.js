...
router.post('/like', async (req, res) => {
  ...
  try {
    ...
    console.log(isLike);
    const updateLikeCount = await Post.findByIdAndUpdate(postId, { $inc: { likeCount: isLike ? 1 : -1 } })
    ...
    if (likeDoc) {
      ...
      if (isPostIdAleady) {
        const likeListDel = await Like.findOneAndUpdate({ userId }, { $pull: { postId: postId } })
        console.log('좋아요 취소', likeListDel);

      } else {
        const likeListUpdate = await Like.findOneAndUpdate({ userId }, { $push: { postId: postId } })
...
router.put('/updateReview/:postId', async (req, res) => {
  ...
  try {
    const updateReview = await Post.findByIdAndUpdate(postId, { $inc: { [updateField]: updateCount } }, { new: true })
...
router.put('/saveCoordiRevw', async (req, res) => {
  ...
  try {
    ...
    if (coordiReviewDoc) {
      ...
      const crdiRvUpdate = await CoordiReview.findOneAndUpdate({ userId, postId }, { btnType }, { new: true })
